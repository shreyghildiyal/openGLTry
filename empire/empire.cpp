...
std::map<int, Empire*> Empire::loadEmpires(std::map<int, Star*> stars, std::map<int, Planet*> planets) {
    ...
    std::string empiresFile = "staticFiles/gameData/empires.json";  // change to avoid hardcoding

    std::ifstream ifs(empiresFile);

    if (ifs.fail()) {
        std::cout << "Failed to read empires file \n";
        return empires;
    }
    ...
    if (jf.is_array()) {
        for (int i = 0; i < jf.size(); i++) {
            ...
            sf::Color colour(jf[i]["colour"]["r"], jf[i]["colour"]["g"], jf[i]["colour"]["b"],
                             jf[i]["colour"]["alpha"]);
...
sf::Color Empire::getColour() { return colour; }
